...
@bp_admin.route('/queue_monitor', methods = ['GET', 'POST'])
@login_required
def queue_monitor():
    logger_bp_admin.info('- in queue_monitor -')
    # logger_bp_admin.info(f"current_user.admin: {current_user.admin_permission}")
    ...
    # Current Jobs
    filename_and_path_current = os.path.join(current_app.config.get('DIR_APPLE_SERVICE_HELPERS'),'current_jobs.json')
    # filename_and_path_current = os.path.join(current_app.config['DIR_APPLE_SERVICE_HELPERS'],'current_jobs.json')
    if os.path.exists(filename_and_path_current):
        with open(filename_and_path_current, 'r') as file:
            dict_jobs_current = json.load(file)
    
    # Complete Jobs
    filename_and_path_complete = os.path.join(current_app.config.get('DIR_APPLE_SERVICE_HELPERS'),'completed_jobs.json')
    if os.path.exists(filename_and_path_complete):
        with open(filename_and_path_complete, 'r') as file:
            dict_jobs_complete = json.load(file)
        



    return render_template('admin/queue_monitor.html',dict_jobs_current=dict_jobs_current,
        dict_jobs_complete=dict_jobs_complete)
